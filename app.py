...
def generate_sku(product):
    """根据品牌生成 SKU，品牌英文字母（最多4位）+ 6位产品ID"""
    brand = product.brand or ''
    # 提取英文字母并转为大写
    letters = ''.join(filter(str.isalpha, brand)).upper()
    if letters:
        prefix = letters[:4]   # 缩进4个空格
    else:
        prefix = 'BRD'         # 缩进4个空格
...
def check_stock_before_ship(items, extra_items=None):
    """
    检查本次发货的产品库存是否足够。
    items: 字典列表，每项至少包含 product_id, quantity
    extra_items: 额外产品列表（可选），格式同上
    返回 (is_ok, error_messages)
    """
    # 合并所有产品数量
    required = {}
    for it in items:
...
def manage_customers():
    if request.method == 'POST':
        ...
        Customer.create(name=name, contact=contact or None, phone=phone or None, address=address or None)
    ...
    query = Customer.select().order_by(Customer.id.desc())
    total = query.count()
    total_pages = max(1, (total + per_page - 1) // per_page)
...
def edit_customer(customer_id):
    ...
    customer.contact = request.form.get('contact', '') or None
