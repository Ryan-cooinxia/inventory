...
- 每次大改动后在 `G:\inventory\项目改动日志\` 目录下记录，命名格式 `YYYY-MM-DD_描述.md`
- 最新日志：[2026-06-21_22_OZON模块全面优化.md](项目改动日志/2026-06-21_22_OZON模块全面优化.md)
- **所有代码改动（不限于某个模块）都必须同步更新改动日志，记录：日期、改动范围、新增/修复内容、影响文件**
- 如果有对应功能的需求文档（如 Excel），也应同步更新
- **项目开发文档统一存放在 `G:\inventory\项目改动日志\`，不得散落到其他目录；文件名使用 `YYYY-MM-DD_主题.扩展名`**
- **AI 生图功能的主开发文档为 `项目改动日志/2026-06-25_AI生图商品理解与事实档案开发文档.xlsx`**
- **今后每次修改 AI 生图相关的需求、数据模型、页面、服务、提示词、模型配置、母图、参考图、生成、QA 或发布流程，都必须同步更新该 Excel 中的任务状态、验收结果、问题风险和“开发日志”工作表**
- **一次 AI 生图改动只有在代码、测试证据、Markdown 改动日志和主开发 Excel 均更新后，才视为完成**
